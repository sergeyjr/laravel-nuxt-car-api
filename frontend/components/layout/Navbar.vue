...
import LogoutModal from '~/components/modals/auth/LogoutConfirmModal.vue'
...
const onLogout = async () => {
    debugLog('[Header] logout click')
    if (isLogoutLoading.value) {
        debugLog('[Header] logout locked')
        return
    }
    ...
    try {
        ...
        if (!ok) {
            debugLog('[Header] logout failed')
            return
        }
        debugLog('[Header] logout success')
